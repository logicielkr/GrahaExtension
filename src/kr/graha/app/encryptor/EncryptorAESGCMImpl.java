 ...
 */
	...
	public String encrypt(String plain) throws NoSuchProviderException {
		return this.encrypt(plain, this.pwd, this.iv);
	}
	public String encrypt(String plain, String pwd, String iv) throws NoSuchProviderException {
		if(Double.valueOf(System.getProperty("java.specification.version")) > 1.7) {
			AESGCMCipher aes = AESGCMCipher.getInstance();
			try {
				return aes.encrypt(plain, pwd, iv);
 ...
 */
	...
	public String decrypt(String encrypted) throws NoSuchProviderException {
		return this.decrypt(encrypted, this.pwd, this.iv);
	}
	public String decrypt(String encrypted, String pwd, String iv) throws NoSuchProviderException {
		if(Double.valueOf(System.getProperty("java.specification.version")) > 1.7) {
			AESGCMCipher aes = AESGCMCipher.getInstance();
			try {
				return aes.decrypt(encrypted, pwd, iv);
			} catch (InvalidKeyException | UnsupportedEncodingException | NoSuchAlgorithmException | NoSuchPaddingException
					| InvalidAlgorithmParameterException | IllegalBlockSizeException | BadPaddingException e) {
				e.printStackTrace();
			}
